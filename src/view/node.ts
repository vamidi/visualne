...
export class NodeView extends Emitter<EventsTypes> {
  ...
  constructor(node: Node, component: Component, emitter: Emitter<EventsTypes>) {
    ...
    this.editorView = <EditorView>emitter;
    this.node = node;
  ...
  onSelect(e: MouseEvent): void {
    ...
    this.onStart();
    // if we have already selected this node we need to deselect the node
    if (e.ctrlKey && this.editorView.nodeEditor.selected.contains(this.node))
    {
      this.trigger("multideselectnode", payload);
      this.trigger("deselectnode", payload);
      return;
    }
